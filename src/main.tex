...
\usepackage[
	...
	%showframe % Debugging frames, uncomment to show frames around margins
]{geometry}
\usepackage[oneside]{sidenotes} % Required for outputting content to the margin\

% Restyle \sidenote command from sidenotes package to reduce the font size
\makeatletter
...
\makeatother
% Environment to output to the full page width (excluding page margins)
\newenvironment{fullwidth}{ 
	% Pull into the right margin by the margin width and margin skip width
	\begin{adjustwidth}{}{\dimexpr-\marginparwidth-\marginparsep\relax} 
}{
	\end{adjustwidth}
}
% Environment to output content at two-thirds of the whole page width
\newenvironment{twothirdswidth}{ 
	% Pull into the right margin by half the margin width and half the margin skip width
	\begin{adjustwidth}{}{\dimexpr-0.5\marginparwidth-0.5\marginparsep\relax} 
}{
	\end{adjustwidth}
}

% Calculate the full text width (including the large right margin and margin padding)
\newlength{\fulltextwidth}
\setlength{\fulltextwidth}{\dimexpr\textwidth+\marginparwidth+\marginparsep\relax}
% New command to output an unnumbered sidenote, the optional argument is a vertical offset and the required parameter is the sidenote text
\newcommand{\nonumsidenote}[2][]{\sidenote[\hspace{0pt}][#1]{#2}} 
% New command to output a quotation sidenote, the optional argument is a vertical offset and the required parameter is the quotation text
\newcommand{\sidenotequote}[2][]{\sidenote[\hspace{0pt}][#1]{\rmfamily\itshape#2}} 
%-------------------------------------------------------
%	FONTS
%-------------------------------------------------------
...
% ----------Rule Definations---------- {el}{l}{sb}
\newcommand{\textel}[1]{{\fontseries{el}\selectfont #1}} % ExtraLight weight command
...
\usepackage{multicol} % Used for multipule colums
...
%	TABLES
%-------------------------------------------------------
\usepackage{booktabs}   % Required for better horizontal rules in tables (\toprule/bottomrule/addlinespace)
\usepackage{rotating}   % Rotating table labels
\usepackage{array} 	    % Required for manipulating table columns
\usepackage{tabularray} % LaTeX3, used for colspec / rowspec
\renewcommand{\arraystretch}{1.25} % Increase the height of table rows

% ----------Rule Definations----------
\newcolumntype{R}[1]{>{\raggedleft\arraybackslash}p{#1}}  % New right-aligned paragraph column type
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}} % New left-aligned (no justification) paragraph column type
\newcolumntype{C}[1]{>{\centering\arraybackslash}p{#1}}   % New centered paragraph column type
...
%	REPORT INFORMATION
%------------------------------------------------
\reporttitle{CompTIA Security+} 			    % The report title to appear on the title page and page headers
\reportsubtitle{Personal Notes {\color{red} Exam Objectives}} % Report subtitle, include new lines if needed
...
\rightheadercontent{\includegraphics[width=3cm]{logo.png}} 					% The content in the right header, leave empty for no right header content
...
\begin{fullwidth}     % Use the whole page width
	...
	%\hfill\includegraphics[width=5cm]{logo.png} % Company logo
	\vspace{0.15\textheight} 				   % Vertical whitespace
	% Report title, intentionally at less than full width for nice wrapping. Adjust width of \parbox and the font size as needed for your title to look good.
	\parbox{1.9\fulltextwidth}{\fontsize{50pt}{52pt}\selectfont\raggedright\textbf{\reporttitle}\par} 
...
%------------------------------------------------
%	SECTION 1: SUBSECTIONS: 1-8
%------------------------------------------------
\section{Threats, Attacks and Vulnerabilities}
\input{sections/1_1AttackTypes}
\input{sections/1_2CompromiseInficators}
\input{sections/1_3ApplicationAttks}
\input{sections/1_4NetworkAttacks}
\input{sections/1_5ThreatActors}
\input{sections/1_6SecurityConcerns}
\input{sections/1_7assessmentTechniques}
\input{sections/1_8PenTest}
